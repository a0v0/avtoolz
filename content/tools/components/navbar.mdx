...
url: https://nextui.org/tools/components/navbar
---
...
| **gap**                      | `number` `string` [CSSGapUnit](/tools/theme/default-theme#spacing) | The gap between each content item. It's `0px` for highlight variants.            | `$10`            |
...
| **LinkProps**     | [LinkProps](/tools/components/link#link-props) | Since dropdown navbar link uses the `Link` component as a base, you can use any of the `Link` props.           | -       |
...
> **Note**: `fluid` means the navbar will fill the entire width of the screen. You can see the complete breakpoint list [here](https://nextui.org/tools/theme/default-theme#breakpoints).
...
> **Note**: You can see the complete media breakpoint list [here](/tools/theme/media#media-queries).
...
> **Note**: For more information about the transition matrix, check the [transition matrix documentation](https://developer.mozilla.org/en-US/tools/Web/CSS/transform-function/matrix).
